...
const Input = forwardRef<HTMLInputElement, Props>(({label, error, ...props}, ref) => {
    ...
    return (
        <div className="relative m-3">
            <label
                className="background-light900_dark200 flex gap-2 absolute -top-3 left-5 rounded-xl px-3 dark:text-light-900">
                {label}
            </label>
            <div>
                <input ref={ref}
                ...
                {label === '* Hasło' &&
                    <button
                        ...
                        className="hover:text-primary-500 absolute top-[21px] right-[20px] z-20"
                    >  {showPassword ? (
                        <Image alt="icon of eye - to hide password" src="/assets/icons/eye-off.svg" width={23}
                               height={20}/>) : (
                        <Image alt="icon of eye - to see password" src="/assets/icons/eye.svg" width={23}
                               height={20}/>)}
                    </button>}
            </div>
