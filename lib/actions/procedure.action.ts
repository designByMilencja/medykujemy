...
export async function createProcedure(params: CreateProcedureParams) {
    try {
        ...
        console.log('dane z tworzenia procedury', title, description,image,sources,desc,video, path)

        // create the procedure
        const procedure = await Procedure.create({
            title, description, image, desc, video,
        });
...
export async function editProcedure(params: EditProcedureParams) {
    try {
        await connectToDatabase();
        const { title, image, description, desc, video, procedureId, path } = params;
