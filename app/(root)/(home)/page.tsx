    ...
    <>
      <div className="flex w-full flex-col-reverse justify-between gap-4 sm:flex-row sm:items-center">
        <h1 className="h1-bold text-dark100_light900">Aktualności medyczne</h1>
          ...
          { session?.user?.role === "admin" ?  <Link href="/articles/add" className="flex justify-end max-sm:w-full">
              <button className="primary-gradient m-2 min-h-[46px] rounded-lg px-4 py-3 text-light-900">Dodaj artykuł</button>
          </Link> : ""}
      </div>
      ...
      <div className="mt-1 flex w-full flex-col gap-6">
        {result.articles.map((article) => (
          <Article key={article._id} desc={article.desc} _id={article._id} title={article.title} src={article.image} tags={article.tags} createdAt={article.createdAt} />
        ))}
      </div>
    </>
  )
